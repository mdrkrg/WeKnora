...
// stubCatalogUserService overrides only the method UserCatalog forwards; the embedded
// interface satisfies the adapter's lazy type assertion.
type stubCatalogUserService struct {
	interfaces.UserService
	byEmail map[string]*types.User
}

func (s *stubCatalogUserService) GetUserByEmail(_ context.Context, email string) (*types.User, error) {
	return s.byEmail[email], nil
}

func TestUserCatalogForwardsLookup(t *testing.T) {
	svc := &stubCatalogUserService{byEmail: map[string]*types.User{
