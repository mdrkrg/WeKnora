...
func TestTicketStoreDeleteExpiredTombstonePhases(t *testing.T) {
	...
	require.NoError(t, store.Create(ctx, &Ticket{TokenHash: "expired-unused", UserID: "u", ExpiresAt: now.Add(-time.Minute)}))
	within := now.Add(-time.Hour)
	require.NoError(t, store.Create(ctx, &Ticket{TokenHash: "tombstone-kept", UserID: "u", ExpiresAt: now.Add(time.Hour), ConsumedAt: &within}))
	old := now.Add(-(ticketTombstoneTTL + time.Hour))
	require.NoError(t, store.Create(ctx, &Ticket{TokenHash: "tombstone-purged", UserID: "u", ExpiresAt: now.Add(-time.Minute), ConsumedAt: &old}))
