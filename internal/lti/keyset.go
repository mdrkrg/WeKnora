...
// keysetRefreshCooldown bounds how often an unknown-kid verification miss can
// force an outbound JWKS fetch. /lti/launch is unauthenticated and a caller can
// present a token with an arbitrary kid, so without a floor each request would
// trigger a fetch + DB write; one refetch per window is enough to pick up a
// genuine rotation.
const keysetRefreshCooldown = 30 * time.Second
...
func NewKeysetResolver(regs RegistrationStore, client *http.Client) KeysetResolver {
	...
	return &keysetResolver{
		regs:      regs,
		client:    client,
		cache:     make(map[uint64]keyfunc.Keyfunc),
		lastFetch: make(map[uint64]time.Time),
	}
}
...
func (r *keysetResolver) Refresh(ctx context.Context, reg *Registration) (keyfunc.Keyfunc, error) {
	r.mu.Lock()
	if last, ok := r.lastFetch[reg.ID]; ok && time.Since(last) < keysetRefreshCooldown {
		// A refresh already happened within the cooldown; reuse whatever is
		// cached instead of issuing another outbound fetch.
		if kf := r.cache[reg.ID]; kf != nil {
			r.mu.Unlock()
			return kf, nil
		}
	}
...
// any stale cached copy. Fetches are rate-limited per registration so an
// unauthenticated caller cannot drive repeated outbound requests by presenting
// tokens with unknown kids.
func (r *keysetResolver) fetchAndPersist(ctx context.Context, reg *Registration) error {
	r.mu.Lock()
	if last, ok := r.lastFetch[reg.ID]; ok && time.Since(last) < keysetRefreshCooldown {
		r.mu.Unlock()
		return fmt.Errorf("lti: jwks refresh throttled")
	}
	r.lastFetch[reg.ID] = time.Now()
	r.mu.Unlock()
