...
// A second refresh inside the cooldown window must reuse the cached keyfunc
// instead of issuing another outbound fetch (unknown-kid requests are
// attacker-controllable on the public launch endpoint).
func TestKeysetResolverRefreshCooldown(t *testing.T) {
	setupSSRFWhitelist(t)
	p := ltitest.NewPlatform(t)
	reg := baseRegistration("https://platform.example.com", "client-1")
	reg.JWKSURI = p.JWKSURL()
	...
	first, err := r.Refresh(context.Background(), reg)
	require.NoError(t, err)
	require.Len(t, regs.savedKeyset, 1)

	second, err := r.Refresh(context.Background(), reg)
	require.NoError(t, err)
	require.Len(t, regs.savedKeyset, 1, "second refresh within cooldown must not refetch")
	require.True(t, keyfuncVerifies(t, second, ltiClaims(p, nil)))
