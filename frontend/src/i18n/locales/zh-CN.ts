export default {
  ...
  datasource: {
    ...
    noResources_canvas: '未找到可同步的 Canvas 课程',
    noResourcesDesc: '应用需要通过群聊获得知识库访问权限才能拉取内容',
    noResourcesDesc_notion: '应用需要获得 Notion 页面的访问权限才能拉取内容',
    noResourcesDesc_canvas: '当前 Canvas 账号没有返回活跃课程，或账号无权访问课程内容',
    ...
    guideStep1_canvas: '确认当前 Canvas 账号已经加入要同步的课程',
    guideStep2_canvas: '确认课程处于活跃状态，并且其中包含可访问的文件',
    guideStep3_canvas: '如课程仍未出现，请返回上一步重新授权后再加载',
    ...
    oauthRevoke: '撤销授权',
    oauthAuthorizeHint: '点击「去授权」，在 Canvas 登录并同意授权。实例地址与 Developer Key 由空间管理员在「设置 → Canvas LMS」中统一配置。',
    oauthAuthorizeFailed: '发起授权失败',
    oauthAuthorizedToast: 'Canvas 授权成功',
    oauthRevokedToast: '已撤销授权',
    oauthRevokeFailed: '撤销授权失败',
    oauthRequired: '请先完成 Canvas OAuth 授权',
    oauthExpired: 'Canvas 授权已失效',
    oauthExpiredDesc: 'Canvas 拒绝了当前凭据。请返回上一步重新授权，再加载课程列表。',
    returnToReauthorize: '返回并重新授权',
    canvasAppUnconfigured: '站点尚未配置 Canvas 应用',
    canvasAppRequired: '请联系站点管理员配置部署环境',
    canvasAppRequiredHint: 'Canvas 实例地址、Client ID / Client Secret 由站点管理员固定，用户只需完成授权。',
    canvasAppGoSettings: '前往管理员设置',
    comingSoon: '即将支持',
    docHint: '在以下地址获取凭证：',
    openDoc: '打开文档',
    ...
    connectorDesc: {
      ...
      canvas: '通过 OAuth2 同步 Canvas 课程文件'
    },
    connector: {
      feishu: '飞书',
      lark: 'Lark（飞书国际版）',
  ...
  settings: {
    ...
    canvasOAuth: {
      ...
      description: '配置本空间对接的 Canvas 实例与 Developer Key。',
      viewDocs: '查看文档',
      unconfigured: '尚未配置 Canvas OAuth 应用凭证',
      configured: 'Canvas OAuth 应用已配置',
      expired: 'Canvas OAuth 凭证已失效',
      expiredDefault: '加密密钥已变更，已保存的 Client Secret 无法解密。请重新填写。',
      reconfigure: '重新配置',
      baseUrlLabel: 'Canvas 实例地址（base_url）',
      baseUrlDesc: 'Canvas 站点根地址，例如 https://canvas.example.com 或本机 http://localhost:3080',
      baseUrlPlaceholder: 'https://canvas.example.com',
      clientIdLabel: 'Client ID',
      clientIdDesc: 'Canvas 管理员后台创建的 Developer Key 的 Client ID',
      clientIdPlaceholder: '请输入 Client ID',
      clientSecretLabel: 'Client Secret',
      clientSecretDesc: 'Developer Key 的 Client Secret，加密存储于本空间',
      clientSecretPlaceholder: '请输入 Client Secret',
      saveHint: '保存后，用户即可在知识库中添加 Canvas 数据源并完成授权。请将 WeKnora 的 /api/v1/datasource/oauth/callback 登记为 Redirect URI。',
      saveBtn: '保存凭证',
      saveSuccess: 'Canvas OAuth 凭证保存成功',
      saveFailed: 'Canvas OAuth 凭证保存失败'
    },
    navGroups: {
      account: '账户',
      workspace: '空间',
  ...
  knowledgeBase: {
    ...
    viewMerged: '全文',
    artifactTab: '解析产物',
    artifactLoading: '正在加载解析产物…',
    artifactLoadError: '解析产物加载失败',
    artifactNoArtifacts: '暂无解析产物，文档可能仍在解析中。',
    artifactResolveImages: '解析图片',
    artifactDownload: '下载',
    artifactDownloadError: '解析产物下载失败',
    artifactContentLoadError: '解析产物内容加载失败',
    artifactTooLarge: '解析产物内容过大，无法内联预览，请下载查看。',
