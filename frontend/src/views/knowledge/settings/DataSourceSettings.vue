...
async function loadList(silent = false) {
  ...
  try {
    ...
    // Keep polling even without a running sync (at a slower cadence) so the
    // list converges on completion/status changes without a manual reload;
    // errors reschedule too so a transient backend hiccup doesn't freeze it.
...
// The editor dialog currently emits `saved` without a payload; once it starts
// carrying the triggered sync log (dialog-side create-and-sync), this becomes
// `(event: DataSourceSyncStartedEvent | null)` and emits 'sync-started' again.
